...
- **Assets are cached, not reactive.** `lib/assets-server.ts` wraps the Convex asset list in `"use cache"` (tagged `assets`) so the root layout can prerender; `AssetsProvider` takes a plain array. A newly added asset needs a redeploy or a `revalidateTag("assets")` to appear. Don't reintroduce `preloadQuery` in `app/layout.tsx` — it forces all 21 routes dynamic.
