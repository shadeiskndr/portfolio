...
export function CommitDiffDialog({
  ...
  const [fileListBySha, setFileListBySha] = useState<Record<string, FileList>>({});
  const [blobsByKey, setBlobsByKey] = useState<Record<string, BlobPair>>({});
  const [selectedPath, setSelectedPath] = useState<string | null>(null);
  const [listError, setListError] = useState<string | null>(null);
  const [blobError, setBlobError] = useState<string | null>(null);

  const sha = commit?.sha;
  const fileList = sha ? fileListBySha[sha] : undefined;

  useEffect(() => {
    if (!sha) return;
    if (fileListBySha[sha]) return;
    let cancelled = false;
    setListError(null);
    fetchCommitFiles({ sha })
      .then((res) => {
        if (cancelled) return;
        setFileListBySha((prev) => ({ ...prev, [sha]: res }));
      })
      .catch((e: unknown) => {
        if (!cancelled) setListError(e instanceof Error ? e.message : "Failed to load files");
      });
    return () => {
      cancelled = true;
    };
  }, [sha, fileListBySha, fetchCommitFiles]);

  useEffect(() => {
    if (!fileList) return;
    setSelectedPath((current) => {
      if (current && fileList.files.some((f) => f.path === current)) return current;
      return fileList.files[0]?.path ?? null;
    });
  }, [fileList]);

  const selectedFile = useMemo(
    () => fileList?.files.find((f) => f.path === selectedPath) ?? null,
    [fileList, selectedPath]
  );

  const blobKey = sha && selectedPath ? `${sha}:${selectedPath}` : null;
  const blobs = blobKey ? blobsByKey[blobKey] : undefined;

  useEffect(() => {
    if (!(sha && fileList && selectedFile && blobKey)) return;
    if (blobsByKey[blobKey]) return;
    let cancelled = false;
    setBlobError(null);
    fetchFileBlobs({
      sha,
      ...
      prevPath: selectedFile.prevPath,
      status: selectedFile.status,
    })
      .then((res) => {
        if (cancelled) return;
        setBlobsByKey((prev) => ({ ...prev, [blobKey]: res }));
      })
      .catch((e: unknown) => {
        if (!cancelled) setBlobError(e instanceof Error ? e.message : "Failed to load file");
      });
    return () => {
      cancelled = true;
    };
  }, [sha, fileList, selectedFile, blobKey, blobsByKey, fetchFileBlobs]);

  const langExt = useMemo(
    () => (selectedFile ? langForPath(selectedFile.path) : null),
    [selectedFile]
  );
        ...
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div className="min-h-0 min-w-0 flex-1 overflow-auto">
              {blobError ? (
                <p className="px-4 py-8 text-center text-destructive text-sm">{blobError}</p>
              ) : !selectedFile ? (
              ...
              ) : blobs.beforeTruncated || blobs.afterTruncated ? (
                <p className="px-4 py-8 text-center text-muted-foreground text-sm">
                  File too large to display. Open on GitHub.
                </p>
              ) : selectedFile.status === "added" || selectedFile.status === "removed" ? (
                <CodeMirror
                  key={blobKey ?? "single"}
                  value={selectedFile.status === "added" ? blobs.after : blobs.before}
                  theme={cmTheme}
